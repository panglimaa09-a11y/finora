...
export interface PaymentAdapter{createTopup(input:TopupRequest):Promise<TopupResponse>;createPayout(input:{amount:number;bank_code:string;account_number:string;account_name:string;reference:string}):Promise<{provider:string;provider_reference:string}>;verifyWebhook(req:Request,rawBody:string):Promise<Record<string,unknown>>;}
