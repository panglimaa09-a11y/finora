...
function isReportPage(root){return root?.querySelector('.dapin-page-head h1')?.textContent?.trim()==='Laporan Keuangan'}
function inject(){
 const root=document.querySelector('.dapin-admin-panel .dapin-main')
 if(!root||!isReportPage(root)||root.querySelector('[data-report-charts]'))return
